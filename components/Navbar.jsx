        ...
        <>
            ...
            <nav ref={navRef} className="w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50">
                ...
                <ul ref={navLinkRef} className="hidden md:flex items-center gap-6 lg:gap-8 rounded-full px-12 py-3 bg-white shadow-sm bg-opacity-50 font-Ovo dark:border dark:border-white/30 dark:bg-transparent ">
                    <li><a className='hover:text-gray-500 dark:hover:text-gray-300 transition' href="#top">{language === 'EN' ? 'Home' : 'Beranda'}</a></li>
                    <li><a className='hover:text-gray-500 dark:hover:text-gray-300 transition' href="#about">{language === 'EN' ? 'About Me' : 'Tentang Saya'}</a></li>
                    <li><a className='hover:text-gray-500 dark:hover:text-gray-300 transition' href="#education">{language === 'EN' ? 'Education' : 'Pendidikan'}</a></li>
                    <li><a className='hover:text-gray-500 dark:hover:text-gray-300 transition' href="#services">{language === 'EN' ? 'Skills' : 'Keahlian'}</a></li>
                    <li><a className='hover:text-gray-500 dark:hover:text-gray-300 transition' href="#work">{language === 'EN' ? 'My Work' : 'Karya Saya'}</a></li>
                </ul>
                ...
                <ul ref={sideMenuRef} className="flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 font-Ovo dark:bg-darkHover dark:text-white">
                    ...
                    <li><a href="#top" onClick={closeMenu}>{language === 'EN' ? 'Home' : 'Beranda'}</a></li>
                    <li><a href="#about" onClick={closeMenu}>{language === 'EN' ? 'About Me' : 'Tentang Saya'}</a></li>
                    <li><a href="#education" onClick={closeMenu}>{language === 'EN' ? 'Education' : 'Pendidikan'}</a></li>
                    <li><a href="#services" onClick={closeMenu}>{language === 'EN' ? 'Skills' : 'Keahlian'}</a></li>
                    <li><a href="#work" onClick={closeMenu}>{language === 'EN' ? 'My Work' : 'Karya Saya'}</a></li>
                </ul>
            </nav>
        </>
    )
