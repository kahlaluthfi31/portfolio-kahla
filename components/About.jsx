...
export default function About() {
  ...
  const topEducationSummary = topEducation ? `${topEducation.institusi}` : "-";
  ...
  const tools = [
    ...
    { name: "RESTful API", icon: "/assets/restfull-api.png" },
  ...
  const data = [
    ...
    {
      name: language === "EN" ? "Work Experience" : "Pengalaman Kerja",
      icon1: "/assets/project-icon.png",
      icon2: "/assets/project-icon-dark.png",
      description:
        language === "EN"
          ? "4-month internship as a Web Developer at Inovasi Alfatih Maulana"
          : "Magang selama 4 bulan sebagai Web Developer di Inovasi Alfatih Maulana",
    },
  ];
  return (
    <div id="about" className="w-full px-[12%] py-10 scroll-mt-20">
      <h4 className="text-center mb-2 text-lg font-Ovo">
        {language === "EN" ? "Introduction" : "Perkenalan"}
      </h4>
      ...
      <div className="flex w-full flex-col lg:flex-row items-center lg:items-start gap-20 my-20">
        ...
        <div className="flex-1">
          <p className="mb-10 max-w-2xl font-Ovo">
            {language === "EN" ? (
              <>
                Kahla Luthfiyah Halim is a{" "}
                <span className="font-bold">Full-Stack Web Developer</span> with
                more than 3 years of experience in software engineering. She is
                skilled at solving complex technical problems and committed to
                delivering optimal software systems in collaborative
                environments.
              </>
            ) : (
              <>
                Kahla Luthfiyah Halim adalah seorang{" "}
                <span className="font-bold">Full-Stack Web Developer</span>{" "}
                dengan pengalaman selama lebih dari 3 tahun di bidang rekayasa
                perangkat lunak. Memiliki kemampuan dalam menyelesaikan masalah
                teknis yang kompleks dan berkomitmen menghasilkan sistem
                perangkat lunak yang optimal dalam lingkungan kerja kolaboratif.
              </>
            )}
          </p>
          ...
          <ul className="flex flex-wrap items-center gap-3 sm:gap-4 max-w-2xl">
            {tools.map((tool) => (
              <li
                key={tool.name}
                className="px-4 py-2 text-sm border border-gray-300 dark:border-white/30 rounded-lg cursor-pointer hover:-translate-y-1 duration-300 hover:bg-lightHover dark:hover:bg-darkHover/50 flex items-center gap-2"
              >
                <img
                  src={tool.icon}
                  alt={tool.name}
